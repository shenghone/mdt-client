...
import { getStudent, handleTo, getOrganizedData } from "../../../util";
...
import { Level1Info, Level2Info, Level3Info } from "./CategoryInfo/Shape";
...
export default ({ location }) => {
  ...
  useEffect(() => {
    if (user && user._id && user.Role === "student") {
      const getCurrentStudent = async () => {
        const d = await getStudent(user._id);
        setCurrentStudent(d);
      };
      getCurrentStudent();
    } else if (user && user._id && user.Role === "teacher" && student) {
  ...
  function populateLevel(item) {
    if (item.Section_name.includes("SS") && item.Answer === item.userAnswer) {
  ...
  const level1Graph = () => {
    ...
    for (i; i < Level1Info.length; i++) {
      if (grade < parseInt(Level1Info[i].Grade)) {
        obj.expected = i;
        break;
      }
    }
            ...
            <div className="mainContainer">
              ...
              <div className="categoryContainer">
                <div className="categoryTitle">
                  Shapes and Space: Shapes/Objects (SS)
                </div>
                ...
                <div className="categoryInfo">
                  <div className="level">
                    ...
                    <h3>Result:</h3>
                    {Level1.length !== 0 ? (
                      <CategoryTable data={Level1} max={4} category={"SS."} />
                    ) : Level2.length !== 0 ? (
                  ...
                  <div className="level">
                    ...
                    <h3>Result:</h3>
                    {Level2.length !== 0 ? (
                      <CategoryTable data={Level2} max={5} category={"SS."} />
                    ) : Level1.length !== 0 ? (
                      <div className="resultText">Level Not Achieved</div>
                    ) : Level3.length !== 0 ? (
                      <div className="resultText">Cleared in Pretest</div>
                  ...
                  <div className="level">
                    ...
                    <h3>Result:</h3>
                    {Level3.length !== 0 ? (
                      <CategoryTable data={Level3} max={2} category={"SS."} />
                    ) : (
                      <div className="resultText">Level Not Achieved</div>
                    )}
                  </div>
